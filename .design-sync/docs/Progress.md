...
Traduz: o `@rivocode/ui-native` exporta `Progress` - `value` de 0 a 100 e `label`; `showValue` e `format` como no web; a barra anda até o valor novo. A API não é a mesma do web (no nativo tudo é controlado), e a [tabela de paridade](/react-native) diz o que muda peça a peça.
