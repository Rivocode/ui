...
function typeAt(input: HTMLInputElement, character: string) {
  ...
  fireEvent.change(input, { target: { value: next, selectionStart: start + 1, selectionEnd: start + 1 } });
}
...
test("o cursor tambem acompanha o campo controlado", () => {
  function Controlled() {
    ...
    return <MaskedInput aria-label="CPF" mask="cpf" value={value} onValueChange={(masked) => setValue(masked)} />;
  }
  render(<Controlled />);
  const input = screen.getByLabelText("CPF") as HTMLInputElement;
  input.focus();
