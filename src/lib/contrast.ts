...
export const CSS_CODE = { ink: "--rc-code-ink", paper: "--rc-code-paper" } as const;

export const MAP_CODE = { ink: "code-ink", paper: "code-paper" } as const;

export function checkCodePair(
  name: string,
...
export const CSS_TINTED_PAIRS: Array<[string, string, number, string, number]> = CSS_SERIES.flatMap(
  (serie) =>
    ["--rc-bg", "--rc-surface"].map(
      (under) => ["--rc-fg", serie, CHART_TINT, under, MIN_TEXT] as [string, string, number, string, number],
    ),
);
...
export function checkSignaturePaper(
  name: string,
  colors: Partial<Record<SignatureRole, string | undefined>>,
): Finding[] {
  const findings: Finding[] = [{ ok: true, line: `\n${name}` }];
  const say = (ok: boolean, line: string) => findings.push({ ok, line: `  ${line}` });

  const missing = (Object.keys(CSS_SIGNATURE) as SignatureRole[]).filter(
    (role) => !colors[role] || !toHex(colors[role]!),
  );
  if (missing.length > 0) {
...
export type TintedPair = { front: string; tint: string; alpha: number; over: string; min: number; note: string };

export const MAP_TINTED_PAIRS: TintedPair[] = [];
