...
1. **A mecânica**, que o script faz: `scripts/audit.ts` lê o JSX, as classes, as
...
1. **Rode o script** sobre o que pediram para auditar:
   ...
   (`node .claude/skills/rivocode-ui-audit/scripts/audit.ts src/pages`), e
   `npx tsx` roda em qualquer Node. O script não tem dependência.
   ...
   `package.json` sai da pasta auditada para cima, até a raiz do repositório,
   e os de cima somam: num monorepo, o peer hoisted na raiz conta.
...
5. **Rode de novo com o JSON** e entregue o relatório que ele imprimir:
   ...
   ```bash
   bun .claude/skills/rivocode-ui-audit/scripts/audit.ts src/pages --julgamento auditoria.json
   ```
...
a linha do arquivo. Descarte que não casa com achado nenhum é recusado, e o
relatório diz qual.
...
|---|---|---|---|
| `cor-literal` | crítico | hexadecimal, `rgb()`, `hsl()`, `oklch()`, classe da paleta do Tailwind (`bg-red-500`, `text-white`), valor arbitrário `bg-[#fff]`, cor por nome em `color`, `fill`, `stroke` | SKILL.md, O que nunca fazer |
| `nome-acessivel` | crítico | `IconButton` sem `label` (nativo: sem `accessibilityLabel`), `Button` ou `<button>` só com ícone e sem nome | a11y.md, Nome acessível |
...
| `elemento-clicavel` | sério | `onClick` em `div`, `span`, `li` e parentes, sem `role` | a11y.md, Foco e teclado |
| `foco-apagado` | sério | `outline-none` sem `focus-visible:ring` na mesma classe | SKILL.md; a11y.md |
| `formulario-sem-zod` | sério | `<form>` num arquivo sem `useZodForm` (busca com `role="search"` passa) | forms.md |
| `dinheiro-float` | sério | `parseFloat`, `Number()` ou `toFixed(2)` numa linha que fala de dinheiro; `type="number"` ou `NumberField` em campo de valor | forms.md, Dinheiro é CurrencyInput |
| `documento-sem-validador` | sério | arquivo que fala de CPF ou CNPJ, valida alguma coisa e não chama `isValidCpf` nem `isValidCnpj` | forms.md |
...
| `peer-faltando` | sério, de projeto | `/chart` sem `recharts`, `/form` sem `react-hook-form` (e sem `zod` e `@hookform/resolvers` quando há `useZodForm`), `/dnd` sem `@dnd-kit/*`, `/editor` sem `@tiptap/*`, a raiz sem `lucide-react`; no nativo, `/chart` sem `react-native-svg`, `/clipboard` sem `expo-clipboard`, `/file-upload` sem `expo-document-picker` | convencoes.md |
...
| `dinheiro-escrito` | moderado | `R$ 12,4K` digitado, `R$ ${valor}`, `Intl.NumberFormat` com `BRL` | SKILL.md, Dinheiro sai abreviado |
...
`audit_screen` recebe os arquivos (caminho e texto), o `package.json` opcional
e o mesmo par `findings` e `dismissals`, e devolve o mesmo relatório, pela
mesma conta.
