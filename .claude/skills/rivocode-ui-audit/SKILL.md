...
| `nome-acessivel` | crítico | `IconButton` sem `label` (nativo: sem `accessibilityLabel`), `Button` ou `<button>` só com ícone e sem nome; no nativo, `Button` e `Pressable` só com ícone e sem `accessibilityLabel`. Nome vazio (`aria-label=""`) não é nome | a11y.md, Nome acessível |
