...
export function Progress({ value, label }: ProgressProps) {
  ...
  return (
    <View
      ...
      className="h-1.5 overflow-hidden rounded-pill bg-skeleton"
    >
...
export function Avatar({ initials, size = "md" }: AvatarProps) {
  ...
  return (
    <View
      className={`items-center justify-center rounded-pill border border-border bg-surface-raised ${box}`}
    >
...
export function Alert({ tone = "info", title, children }: AlertProps) {
  ...
  return (
    <View accessibilityRole="alert" className={`gap-1 rounded-md border p-4 ${styles.box}`}>
